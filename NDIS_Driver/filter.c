...
)
{
    ...
    /*NdisDeviceObject->Flags = DO_BUFFERED_IO;
    NdisDeviceObject->Flags &= ~DO_DEVICE_INITIALIZING;*/
...
)
{
    ...
    PRecord CS = NULL;
    Irp->IoStatus.Information = 0;
    KeAcquireSpinLock(&IC_LOCK, &IC_IRQL);//上锁
    switch (IrpSp->Parameters.DeviceIoControl.IoControlCode)
    {

    case NEW_FC_CODE://新增规则
    {
        if (start_flag == 1)
        {
            Irp->IoStatus.Information = 0;//失败
            break;
        }
        PUCHAR Buffer = NULL;
        ...
        //DWORD32 KK = IrpSp->Parameters.DeviceIoControl.InputBufferLength;
        memcpy(PFC, Buffer, sizeof(Filter_Condition));//读入数据
        //DbgBreakPoint();
        if (PFC->flag == 0)
        {
            ExFreePool(PFC);//没有正确读入，不加入规则
            Irp->IoStatus.Information = 0;
            break;
        }
    ...
    case DEL_FC_CODE://删除规则
    {
        ...
        memcpy(&fflag, Buffer, sizeof(int));//读入数据
        PFilter_Condition ls = FC_list.head;
        if (ls == NULL || FC_list.num == 0)
        {
            Irp->IoStatus.Information = 0;//失败
            break;
        }
        //删除一个或相同标识号的一组规则
        for (ls = FC_list.head; ls != NULL;) {
            if (ls->flag == fflag)
            {
    ...
    case DEL_ALL_FC_CODE://删除所有规则
    {
        ...
        PFilter_Condition LS = FC_list.head;
        PFilter_Condition LS2 = FC_list.head;
        do {
            if (LS == NULL)//清空规则表
                break;
            LS2 = LS->next;
        ...
        FC_list.head = FC_list.last = NULL;
        Irp->IoStatus.Information = 1;
        break;
    }
    case GET_B_FC_CODE://获取恶意会话
    {
        PUCHAR BUFFER= IrpSp->Parameters.DeviceIoControl.Type3InputBuffer;//交互缓冲区
        ULONG LENHTH = IrpSp->Parameters.DeviceIoControl.InputBufferLength;//交互长度
        PRecord LS = M_Rrd_List.head;
        int num = LENHTH / sizeof(Record);//得到的num为需要获取的恶意ip个数
        for (int i = 0; i < num; i++)
        {
            memcpy(BUFFER + i * sizeof(Record), LS, sizeof(Record));
            LS = LS->next;
        }
        Irp->IoStatus.Information = 1;
        break;
    }
    case GET_B_NUM_CODE://获取恶意会话个数
    {
        PUCHAR BUFFER = IrpSp->Parameters.DeviceIoControl.Type3InputBuffer;
        int length = M_Rrd_List.num;
    ...
    KeReleaseSpinLock(&IC_LOCK, IC_IRQL);//解锁
    Irp->IoStatus.Status = Status;
...
--*/
{
    ...
    //Status = IoCreateDevice(DriverObject, 200, &device_name, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN, 0, &mydevice);
    ...
    //    return Status;
    //}
    //DbgPrint("成功创建符号链接。%p", DriverObject);
    //DriverObject->MajorFunction[IRP_MJ_CREATE] = CreateCompleteRoutine;
    //DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = myfun;
    //-------------------------------------------------------------
    ...
    //-----------------------------锁-----------
    KeInitializeSpinLock(&IC_LOCK);
    KeInitializeSpinLock(&IC_LOCK2);
    KeInitializeSpinLock(&IC_LOCK3);
    //-----------------------------锁-----------
    do
    {
...
--*/
{
    ...
    do
    {
        ...
        //
        pFilter->TrackReceives = TRUE;//开启收包过滤
        pFilter->TrackSends = TRUE;//开启发包过滤
...
--*/
{
    ...
    PRecord LSS = B_Rrd_List.head; //释放 温和会话表
    PRecord LSS2 = B_Rrd_List.head;
    do {
        if (LSS == NULL)//清空规则表
            break;
        LSS2 = LSS->next;
    ...
    PRecord LSSS = M_Rrd_List.head; //释放 危险会话表
    PRecord LSSS2 = M_Rrd_List.head;
    do {
        if (LSSS == NULL)//清空规则表
            break;
        LSSS2 = LSSS->next;
...
--*/
{
    ...
    BOOLEAN             DispatchLevel;
    BOOLEAN             bFalse = FALSE;
    PNET_BUFFER_LIST    pCurrentNetBuffer;
    pCurrentNetBuffer = NetBufferLists;
...
//#endif
        ...
        PPacket need_free = NULL;
        Packet_Information LSPI = { 0 };

        //PNET_BUFFER pnb = NET_BUFFER_LIST_FIRST_NB(NetBufferLists);
        //need_free = IC_Get_Packer(pnb);
        //if (need_free == NULL)
        //    return;
        //PTP_ETHERNET PET = IC_Get_Data_Link_Layer(need_free);
        //Network_Layer NL = IC_Get_Network_Layer(need_free, PET, &LSPI);
        //if (Network_Layer_Is_Space(NL) == 1)//是否为未定义的网络层
        //{
        //    ExFreePool(need_free);
        //}
        //else
        //{
        //    //DbgBreakPoint();
        ...
        if (start_flag == 1)
        {
            if (Send_Filtering_Function(NetBufferLists, &need_free, &LSPI) == 1)
            {
                //Send_Record_IP(&LSPI);//拦截的不记录
                NdisFSendNetBufferListsComplete(pFilter->FilterHandle,
                    NetBufferLists,
                    DispatchLevel ? NDIS_SEND_COMPLETE_FLAGS_DISPATCH_LEVEL : 0);//拦截
                ExFreePool(need_free);
            }
            else {
                Send_Record_IP(&LSPI);//记录会话
                if (need_free != NULL)
                    ExFreePool(need_free);
...
--*/
{
    ...
    {/*
        ...
        ///
        //if(start_flag==1)
        //    if(Rec_Filtering_Function(NetBufferLists)==1)
        //        NdisFReturnNetBufferLists(pFilter->FilterHandle, NetBufferLists, 0);//拦截
        ...
        //    }
        //    PTP_ETHERNET PET = IC_Get_Data_Link_Layer(need_free);
        //    Network_Layer NL = IC_Get_Network_Layer(need_free, PET, &RLSPI);
        //    if (Network_Layer_Is_Space(NL))//是否为未定义的网络层
        //    {
        //        ExFreePool(need_free);
        //    }
        //    else
        ...
        if (start_flag == 1)
        {
            if (Rec_Filtering_Function(NetBufferLists, &need_free, &RLSPI) == 1)
            {
                //Rec_Record_IP(&RLSPI);//拦截的不记录
                ULONG               returnflags = 0;
                NDIS_SET_RETURN_FLAG(returnflags, NDIS_RETURN_FLAGS_DISPATCH_LEVEL);
                //DbgBreakPoint();
                NdisFReturnNetBufferLists(pFilter->FilterHandle, NetBufferLists, returnflags);//拦截
                ExFreePool(need_free);
                break;
            }
            else
