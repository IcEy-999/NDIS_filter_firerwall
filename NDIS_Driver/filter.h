...
#define FILTER_UNIQUE_NAME          L"{91E009E2-A9A3-42B7-9E22-3A1D389D7D4D}" //unique name, quid name
...
typedef struct _MS_FILTER
{
    ...
    NDIS_HANDLE                     FilterHandle;//过滤驱动句柄
    ...
    NDIS_STRING                     FilterName;//过滤驱动名
    ULONG                           CallsRestart;
    BOOLEAN                         TrackReceives;//收包过滤
    BOOLEAN                         TrackSends;//发包过滤
#if DBG
    BOOLEAN                         bIndicating;
#endif
